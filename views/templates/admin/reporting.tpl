            ...
            <form method="get" action="{$action_url}" class="form-inline">
                ...
                <div class="col-sm-12 col-md-8 pts-reporting-filters-box">
                    <div class="form-group col-sm-12 col-md-6">
                        ...
                        <select id="pts-month-from" name="month_from" class="form-control">
                            {foreach from=$months item=opt}
                                <option value="{$opt.value}" {if $opt.value == $month_from}selected{/if}>{$opt.label}
                                </option>
                            {/foreach}
                        </select>
                    ...
                    <div class="form-group col-sm-12 col-md-6 pts-reporting-spaced-group">
                        ...
                        <input id="pts-depannage-rate" type="number" name="depannage_rate" value="{$depannage_rate}"
                            min="0.01" step="0.01" class="form-control">
                    </div>

                    <div class="form-group col-sm-12 col-md-6 pts-reporting-spaced-group">
                        <label
                            for="pts-report-emails">{l s='Email(s) (separes par virgule)' mod='ps_pts_reporting'}</label>
                    ...
                    <div class="row">
                        <div class="col-xs-3">
                            <select name="report_monthly_month" class="form-control">
                                {foreach from=$months item=opt}
                                    <option value="{$opt.value}" {if $opt.value == $report_monthly_month}selected{/if}>
                                        {$opt.label}
                                    </option>
                                {/foreach}
                            </select>
        ...
        <div class="tab-pane{if $active_tab == 'tab-kpi-clients'} active{/if}" id="tab-kpi-clients">
            ...
            <form method="get" action="{$action_url}" class="form-inline pts-kpi-clients-form">
                ...
                <div class="form-group">
                    <label class="radio-inline">
                        <input type="radio" name="kpi_view_mode" value="detail"
                            {if $kpi_view_mode == 'detail'}checked{/if}> {l s='Détail' mod='ps_pts_reporting'}
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="kpi_view_mode" value="cumul"
                            {if $kpi_view_mode == 'cumul'}checked{/if}> {l s='Cumul' mod='ps_pts_reporting'}
                    </label>
                </div>
            ...
            {if !empty($customer_kpi_rows)}
                <div class="pts-kpi-summary">
                    ...
                    <div class="col-xs-12 col-sm-6 col-md-3">
                        <div class="pts-kpi-stat-block">
                            <span
                                class="pts-kpi-stat-label">{l s='Devis / Cdes / Transfo' mod='ps_pts_reporting'}</span>
                            <span class="pts-kpi-stat-value">{$customer_kpi_summary.total_devis} /
                                {$customer_kpi_summary.total_cmds} / {$customer_kpi_summary.total_transfo}</span>
                            <span class="pts-kpi-stat-sub">{l s='Taux transfo' mod='ps_pts_reporting'} :
                                {$customer_kpi_summary.taux_transfo} % &mdash; panier :
                                {$customer_kpi_summary.panier_moyen} € &mdash; avoirs :
                                {$customer_kpi_summary.total_avoirs} &mdash; nouveaux :
                                {$customer_kpi_summary.nb_nouveaux}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="pts-kpi-clients-table-wrap">
                <table class="table table-bordered table-hover table-condensed pts-kpi-clients-table">
                    ...
                    <tbody>
                        {if $kpi_view_mode == 'cumul'}
                            <tr>
                                <td class="text-right">{$customer_kpi_summary.total_ca_n}</td>
                                <td class="text-right text-muted">{$customer_kpi_summary.total_ca_n1}</td>
                                <td
                                    class="text-right {if $customer_kpi_summary.ecart_ca > 0}text-success{elseif $customer_kpi_summary.ecart_ca < 0}text-danger{/if}">
                                    {$customer_kpi_summary.ecart_ca}</td>
                                <td
                                    class="text-right {if $customer_kpi_summary.pct_ca_vs_n1 > 0}text-success{elseif $customer_kpi_summary.pct_ca_vs_n1 < 0}text-danger{/if}">
                                    {$customer_kpi_summary.pct_ca_vs_n1}&nbsp;%</td>
                                <td class="text-right">{$customer_kpi_summary.total_mb_n}</td>
                                <td class="text-right">{$customer_kpi_summary.pct_mb_n}&nbsp;%</td>
                                <td class="text-right text-muted">{$customer_kpi_summary.total_mb_n1}</td>
                                <td class="text-right text-muted">{$customer_kpi_summary.pct_mb_n1}&nbsp;%</td>
                                <td
                                    class="text-right {if $customer_kpi_summary.pct_mb_vs_n1 > 0}text-success{elseif $customer_kpi_summary.pct_mb_vs_n1 < 0}text-danger{/if}">
                                    {$customer_kpi_summary.pct_mb_vs_n1}&nbsp;%</td>
                                <td class="text-right">{$customer_kpi_summary.total_devis}</td>
                                <td class="text-right">{$customer_kpi_summary.total_cmds}</td>
                                <td class="text-right">{$customer_kpi_summary.total_transfo}</td>
                                <td class="text-right">{$customer_kpi_summary.taux_transfo}&nbsp;%</td>
                                <td class="text-right">{$customer_kpi_summary.panier_moyen}</td>
                                <td class="text-right">{$customer_kpi_summary.total_avoirs}</td>
                            </tr>
                        {else}
                            {foreach from=$customer_kpi_rows item=row}
                                <tr{if $row.is_new_customer} class="pts-new-customer" {/if}>
                                    ...
                                    <td
                                        class="text-right {if $row.ecart_ca > 0}text-success{elseif $row.ecart_ca < 0}text-danger{/if}">
                                        {$row.ecart_ca}</td>
                                    <td
                                        class="text-right {if $row.pct_ca_vs_n1 > 0}text-success{elseif $row.pct_ca_vs_n1 < 0}text-danger{/if}">
                                        {$row.pct_ca_vs_n1}&nbsp;%</td>
                                    ...
                                    <td
                                        class="text-right {if $row.pct_mb_vs_n1 > 0}text-success{elseif $row.pct_mb_vs_n1 < 0}text-danger{/if}">
                                        {$row.pct_mb_vs_n1}&nbsp;%</td>
